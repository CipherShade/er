...
/** Date offset by `days` from today at the given hour */
...
async function main(): Promise<void> {
  ...
  const admin = await prisma.user.upsert({
    ...
    create: {
      username: 'admin', email: 'admin@alawael.local',
      passwordHash: await hashPwd(adminPassword),
      fullName: 'أ/ محمود الشريف', role: Role.ADMIN,
      phoneNumber: '01000000000', preferredLanguage: 'ar',
      isActive: true, tenantId: tenant.id,
    },
  });
  console.log(`✅  Admin      : ${admin.fullName} (admin / Admin@12345!)`);

  const receptionist = await prisma.user.upsert({
    where:  { username: 'reception1' },
    update: { tenantId: tenant.id },
    create: {
      username: 'reception1', email: 'reception1@alawael.local',
      passwordHash: await hashPwd(receptionistPassword),
      fullName: 'سارة عبد الرحمن', role: Role.RECEPTIONIST,
      phoneNumber: '01012345678', preferredLanguage: 'ar',
      isActive: true, tenantId: tenant.id,
    },
  });
  console.log(`✅  Reception  : ${receptionist.fullName} (reception1 / Desk@12345!)`);
  ...
  rooms.forEach((r) => console.log(`✅  Room       : ${r.name} (${r.capacity} مقعد)`));
  ...
  for (const t of teachersRaw) {
    ...
    teachers.push(await prisma.teacher.create({
      data: {
        tenantId: tenant.id, fullName: t.fullName, searchName: norm(t.fullName),
        phoneNumber: t.phone, subject: t.subject, defaultCenterFee: t.fee,
        assistantName: t.asst, assistantPhone: t.asstPhone, isActive: true,
      },
    }));
  }
  teachers.forEach((t) => console.log(`✅  Teacher    : ${t.fullName} (${t.subject})`));
  ...
  for (let i = 0; i < studentsRaw.length; i++) {
    ...
    students.push(await prisma.student.create({
      data: {
        tenantId: tenant.id, studentCode: code,
        fullName: s.n, searchName: norm(s.n),
        studentPhone: s.p, guardianPhone: s.g,
        academicStage: s.stage, schoolType: s.type,
      },
    }));
  }
  console.log(`✅  Students   : ${students.length} طالب`);
  ...
  for (const sp of shiftsSpec) {
    shifts.push(await prisma.shiftRegister.create({
      data: {
        tenantId:          tenant.id,
  ...
  console.log(`✅  Shifts     : ${shifts.length} وردية مغلقة`);
  ...
  interface SessionSpec {
    teacher: TeacherRow; room: typeof roomBig;
    title: string; stage: string; price: number; fee: number;
    startAt: Date; endAt: Date; shift: ShiftRow;
    attendees: StudentRow[];
    pmCycle: ('CASH'|'VODAFONE_CASH'|'INSTAPAY')[];
    assistantCount?: number; recoNotes?: string;
  }

  async function makeSession(sp: SessionSpec): Promise<SessionRow> {
    const session = await prisma.session.create({
      data: {
        tenantId: tenant.id, teacherId: sp.teacher.id, roomId: sp.room.id,
        title: sp.title, academicStage: sp.stage,
        startTime: sp.startAt, endTime: sp.endAt,
    ...
    for (let i = 0; i < sp.attendees.length; i++) {
      ...
      await prisma.attendance.create({
        data: {
          tenantId: tenant.id, sessionId: session.id, studentId: sp.attendees[i].id,
          receptionistId: receptionist.id, shiftRegisterId: sp.shift.id,
    ...
    const recon = await prisma.sessionReconciliation.create({
      data: {
        tenantId: tenant.id, sessionId: session.id,
        lobbyCount, assistantCount, discrepancy,
        reconciledHeadcount: reconciledHead,
        resolutionNotes: sp.recoNotes ?? null,
        reconciledById: admin.id, reconciledAt: sp.endAt,
      },
    });
    ...
    await prisma.sessionSettlement.create({
      data: {
        tenantId: tenant.id, sessionId: session.id,
        reconciliationId: recon.id, disbursedFromShiftId: sp.shift.id,
        reconciledHeadcount: reconciledHead,
        sessionPrice: new Prisma.Decimal(sp.price),
        centerFeePerStudent: new Prisma.Decimal(sp.fee),
        ...
        payoutMethod: 'CASH', recipientName: sp.teacher.fullName,
        status: 'DISBURSED', createdById: admin.id, settledAt: sp.endAt,
      },
    });
  ...
  sessions.push(await makeSession({ teacher:tPhysics, room:roomBig,
    title:'الفيزياء — موجات وصوت (الثالث الثانوي)', stage:'الثالث الثانوي',
    price:150, fee:20, startAt:daysAgo(14,9), endAt:daysAgo(14,11),
    shift:sh1, attendees:grade3.slice(0,8), pmCycle:['CASH','CASH','VODAFONE_CASH','CASH'],
    assistantCount:8 }));

  sessions.push(await makeSession({ teacher:tMath, room:roomMid,
    title:'الرياضيات — حساب التفاضل (الثالث الثانوي)', stage:'الثالث الثانوي',
    price:120, fee:15, startAt:daysAgo(13,15), endAt:daysAgo(13,17),
    shift:sh2, attendees:grade3.slice(1,10), pmCycle:['CASH','INSTAPAY','CASH','VODAFONE_CASH'],
    assistantCount:10, recoNotes:'اعتُمد عدد الاستقبال' }));

  sessions.push(await makeSession({ teacher:tChem, room:roomBig,
    title:'الكيمياء — الروابط الكيميائية (الثاني الثانوي)', stage:'الثاني الثانوي',
    price:130, fee:25, startAt:daysAgo(10,9), endAt:daysAgo(10,11),
    shift:sh3, attendees:grade2.slice(0,10), pmCycle:['CASH','CASH','CASH','VODAFONE_CASH','INSTAPAY'] }));

  sessions.push(await makeSession({ teacher:tPhysics, room:roomBig,
    title:'الفيزياء — الضوء والبصريات (الثالث الثانوي)', stage:'الثالث الثانوي',
    price:150, fee:20, startAt:daysAgo(7,9), endAt:daysAgo(7,11),
    shift:sh4, attendees:grade3, pmCycle:['CASH','CASH','VODAFONE_CASH','INSTAPAY','CASH'] }));

  sessions.push(await makeSession({ teacher:tArabic, room:roomSmall,
    title:'اللغة العربية — النحو والصرف (الأول الثانوي)', stage:'الأول الثانوي',
    price:100, fee:10, startAt:daysAgo(7,12), endAt:daysAgo(7,14),
    shift:sh4, attendees:grade1.slice(0,8), pmCycle:['CASH','CASH','CASH','VODAFONE_CASH'] }));

  sessions.push(await makeSession({ teacher:tBio, room:roomMid,
    title:'الأحياء — الخلية ووظائفها (الثاني الثانوي)', stage:'الثاني الثانوي',
    price:140, fee:20, startAt:daysAgo(4,9), endAt:daysAgo(4,11),
    shift:sh5, attendees:grade2.slice(2,11), pmCycle:['CASH','INSTAPAY','CASH'] }));

  sessions.push(await makeSession({ teacher:tMath, room:roomSmall,
    title:'الرياضيات — المثلثات (الأول الثانوي)', stage:'الأول الثانوي',
    price:110, fee:15, startAt:daysAgo(4,11), endAt:daysAgo(4,13),
    shift:sh5, attendees:grade1, pmCycle:['CASH','VODAFONE_CASH','CASH'] }));

  sessions.push(await makeSession({ teacher:tChem, room:roomBig,
    title:'الكيمياء — الكيمياء العضوية (الثالث الثانوي)', stage:'الثالث الثانوي',
    price:130, fee:25, startAt:daysAgo(1,9), endAt:daysAgo(1,11),
    shift:sh6, attendees:grade3, pmCycle:['CASH','CASH','INSTAPAY','VODAFONE_CASH'] }));

  console.log(`✅  Sessions   : ${sessions.length} حصة مكتملة`);
  ...
  for (const e of expensesRaw) {
    await prisma.expense.create({
      data: {
        tenantId: tenant.id, shiftRegisterId: e.shift.id,
        category: e.cat, amount: new Prisma.Decimal(e.amt),
        paymentMethod: e.pm, description: e.desc, createdById: receptionist.id,
      },
    });
  }
  console.log(`✅  Expenses   : ${expensesRaw.length} مصروف مسجل`);

  // ══════════════════════════════════════════════════════════════════
  // SUMMARY
  // ══════════════════════════════════════════════════════════════════
  const totalCheckIns  = await prisma.attendance.count({ where: { tenantId: tenant.id } });
  const totalSettled   = await prisma.sessionSettlement.count({ where: { tenantId: tenant.id } });
  ...
  console.log('  ├─────────────────────────────────────────────────────┤');
  console.log('  │  🖥  Reception  username : reception1               │');
  console.log('  │                password : Desk@12345!               │');
  console.log('  └─────────────────────────────────────────────────────┘\n');
  console.log(`  Tenant   : ${tenant.name}`);
  console.log(`  Teachers : ${teachers.length}`);
  console.log(`  Students : ${students.length}`);
  console.log(`  Sessions : ${sessions.length} (جميعها COMPLETED)`);
  console.log(`  Shifts   : ${shifts.length} (جميعها CLOSED)`);
  console.log(`  Check-ins: ${totalCheckIns}`);
  console.log(`  Settled  : ${totalSettled} حصة`);
  console.log(`  Expenses : ${expensesRaw.length}`);
  console.log('─────────────────────────────────────────────────────────────────\n');
}
