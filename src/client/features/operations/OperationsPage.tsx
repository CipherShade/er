...
import { BarChart3, ClipboardCheck, Coins, Search, Wallet } from 'lucide-react';
...
type Shift = { id: string; financials?: { expectedCashInDrawer: number; totalCashCollected: number; totalTeacherCashPayouts: number; totalCashExpenses: number } };
...
export function mapApiErrorMessage(error: unknown, fallback: string): string {
  if (error instanceof ApiError) {
    if (error.code === 'DUPLICATE_CHECK_IN') return 'الطالب مسجل بالفعل في هذه الحصة';
    if (error.code === 'SESSION_CAPACITY_REACHED') return 'هذه الحصة وصلت إلى الحد الأقصى للسعة';
    if (error.code === 'SHIFT_NOT_OPEN') return 'يجب فتح الوردية أولاً';
    if (error.code === 'SHIFT_CLOSED' || error.code === 'SHIFT_ALREADY_CLOSED') return 'تم إغلاق الوردية بالفعل';
    if (error.code === 'SESSION_LOCKED') return 'الحصة منتهية ولا يمكن تعديلها أو تكرار تسويتها';
    if (error.code === 'PAYMENT_REFERENCE_REQUIRED') return 'أدخل رقم العملية لطريقة الدفع الرقمية';
    if (error.code === 'SCHEDULE_CONFLICT') return 'يوجد تعارض في الموعد';
    if (error.code === 'UNAUTHORIZED') return 'غير مصرح لك بالوصول، يرجى تسجيل الدخول';
    if (error.code === 'FORBIDDEN' || error.code === 'AUDIT_ACCESS_DENIED') return 'ليس لديك الصلاحية لتنفيذ هذه العملية';
    if (error.code === 'VALIDATION_ERROR' && error.message && error.message !== 'REQUEST_FAILED' && /[\u0600-\u06FF]/.test(error.message)) {
...
const money = (value: number | undefined) => `${Number(value || 0).toFixed(2)} ج.م`;
...
function LobbyPage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      setError(mapApiErrorMessage(err, t('operations.loadError')));
    } finally {
      setLoading(false);
    }
  };
  ...
  async function checkIn(student: Student) {
    ...
    if (paymentMethod !== 'CASH' && !reference.trim()) {
      setError('أدخل رقم العملية لطريقة الدفع الرقمية');
      return;
    }
    ...
    try {
      ...
      await request('/attendances/checkin', {
        method: 'POST',
      ...
      setError(mapApiErrorMessage(err, t('operations.checkInError')));
    } finally {
              ...
              {paymentMethod === 'CASH' ? (
                <div className="rounded-lg border border-slate-700 bg-slate-950 p-2.5">
                  <div className="flex items-center justify-between text-xs text-slate-400">
                    <span>سعر الحصة: {money(sessionPrice)}</span>
                    <span className="font-bold text-emerald-400">الباقي: {money(changeOwed)}</span>
                  </div>
                  <input
                    type="number"
                    min="0"
                    ...
                    placeholder={sessionPrice ? `المبلغ المستلم (افتراضي: ${sessionPrice})` : 'المبلغ المستلم'}
...
function ShiftPage() {
  ...
  async function submit(path: string, body: object, message: string) {
    ...
    try {
      ...
      setError(mapApiErrorMessage(err, t('operations.saveError')));
    } finally {
      setPending(false);
    }
  }
        ...
        <div className="space-y-4">
          <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
            {[
              ...
              <Metric key={key as string} label={t(`operations.shift.${key as string}`)} value={money(value as number)} />
            ))}
          </div>
          <form
            onSubmit={(event) => {
...
function SessionActionPage({ kind }: { kind: 'reconciliation' | 'settlement' }) {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      await request(`/sessions/${sessionId}/${kind === 'reconciliation' ? 'reconcile' : 'settle'}`, {
        method: 'POST',
        ...
        <form onSubmit={submit} className="max-w-xl space-y-4 rounded-xl border border-slate-800 bg-slate-900 p-5">
          ...
          {kind === 'reconciliation' ? (
            <>
              ...
              <Field label={t('operations.reconciliation.assistantCount')} value={first} onChange={setFirst} type="number" />
              <Field label={t('operations.reconciliation.headcount')} value={second} onChange={setSecond} type="number" />
              <Field label={t('operations.reconciliation.notes')} value={notes} onChange={setNotes} required={false} />
            </>
          ) : (
            <>
              ...
              />
              <Field label={t('operations.settlement.recipient')} value={second} onChange={setSecond} />
            </>
          )}
          <button
        ...
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          {['totalAttendees', 'centerNetRevenue', 'teacherPayouts', 'digitalCollections'].map((key) => (
            <Metric
              ...
              value={key === 'totalAttendees' ? String(report[key]) : money(report[key] as number)}
            />
          ))}
        </div>
      )}
      <div className="max-w-xl rounded-xl border border-slate-800 bg-slate-900 p-5">
        <Field label={t('operations.reports.shiftId')} value={shiftId} onChange={setShiftId} />
        <button
          type="button"
          ...
          <div className="mt-4 space-y-2">
            {entries.map((entry, index) => (
              <div key={`${String(entry.id || index)}`} className="rounded-lg border border-slate-800 p-3 text-sm">
                <span className="font-bold text-emerald-300">{String(entry.action)}</span>
                <span className="ms-3 text-slate-400">{String(entry.createdAt)}</span>
                {entry.amount !== null && <span className="ms-3 text-amber-300">{money(entry.amount as number)}</span>}
              </div>
            ))}
          </div>
        ) : (
